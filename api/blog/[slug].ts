...
async function loadArticle(slug: string): Promise<ArticleRow | null> {
  ...
  const result = await getPool().query<ArticleRow>(
    `SELECT slug, title, content, status, created_at
     FROM articles
     WHERE slug = $1
     LIMIT 1`,
    [normalizedSlug]
  ...
  try {
    ...
    if (!process.env.DATABASE_URL || process.env.DATABASE_URL.trim().length === 0) {
      ...
      res.send(renderPage({
        ...
        seoTitle: 'Database error — Crypto Bot',
        seoDescription: 'Database configuration is missing.',
        body: '<h1>Database error</h1><p style="color:#9fb2d8;">Database error</p>'
      }));
      return;
    }
    ...
    try {
      ...
      console.error('Database error while loading blog article', error);
      res.status(500).setHeader('Content-Type', 'text/html; charset=utf-8');
      res.send(renderPage({
        title: 'Database error',
        seoTitle: 'Database error — Crypto Bot',
        seoDescription: 'Unable to load article from database.',
        body: '<h1>Database error</h1><p style="color:#9fb2d8;">Database error</p>'
      }));
      return;
    }
    ...
    console.error('Unhandled blog handler error', error);
    res.status(500).setHeader('Content-Type', 'text/html; charset=utf-8');
    res.send(renderPage({
      title: 'Server error',
